...
async function verifyTeacherToken(authorizationHeader) {
  ...
  try {
    ...
    // Surface the real Admin SDK error (e.g. project/audience mismatch, expired
    // token, malformed token) instead of a generic message — none of these
    // reasons are sensitive, and guessing blind wastes a redeploy-and-retry cycle.
    const err = new Error(`Invalid or expired sign-in session: ${e.message}`);
    err.statusCode = 401;
    throw err;
  }
